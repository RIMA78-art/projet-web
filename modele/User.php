...
class User {
    ...
    public static function mettreAJour($id, $nom, $prenom, $taille, $poids, $objectif, $niveau_sport) {
        ...
        // Récupérer l'ancien poids pour voir si on doit mettre à jour last_weight_update
        $ancienUser = self::trouverParId($id);
        $poidsChange = $ancienUser && $ancienUser['poids'] != $poids && $poids > 0;

        $stmt = $pdo->prepare(
            "UPDATE utilisateur
             SET nom=?, prenom=?, taille=?, poids=?, objectif=?, niveau_sport=?" .
             ($poidsChange ? ", last_weight_update=NOW()" : "") .
             " WHERE id_user=?"
        );
    ...
    /**
     * Calcule le score d'activité d'un utilisateur (0-100)
     * Critères : activité récente (30%), complétude profil (30%), mise à jour poids (20%), ancienneté (20%)
     */
    public static function calculerScore(array $user): int {
        $score = 0;

        // 1. Activité récente (30 points) - dernière connexion dans les 7 derniers jours
        ...
        if ($lastLogin) {
            ...
            if ($daysSinceLogin <= 7) {
                ...
                $score += 15; // Moins de points si plus ancien
            }
        }

        // 2. Complétude du profil (30 points) - champs obligatoires remplis
        $champsProfil = ['nom', 'prenom', 'email', 'objectif', 'niveau_sport', 'taille', 'poids'];
        $champsRemplis = count(array_filter($champsProfil, fn($champ) => !empty($user[$champ])));
        $score += (int)(($champsRemplis / count($champsProfil)) * 30);

        // 3. Mise à jour du poids récente (20 points) - dans les 30 derniers jours
        $lastWeightUpdate = $user['last_weight_update'] ?? null;
        if ($lastWeightUpdate) {
            $daysSinceWeightUpdate = (time() - strtotime($lastWeightUpdate)) / (60 * 60 * 24);
            if ($daysSinceWeightUpdate <= 30) {
                $score += 20;
            } elseif ($daysSinceWeightUpdate <= 90) {
                $score += 10; // Moins de points si plus ancien
            }
        }

        // 4. Ancienneté (20 points) - inscrit depuis plus de 30 jours
        $dateInscription = $user['date_inscription'] ?? null;
        if ($dateInscription) {
            $daysSinceRegistration = (time() - strtotime($dateInscription)) / (60 * 60 * 24);
            if ($daysSinceRegistration > 30) {
                $score += 20;
            } elseif ($daysSinceRegistration > 7) {
                $score += 10; // Points partiels pour les nouveaux utilisateurs actifs
            }
        }
     ...
     */
    public static function obtenirBadge(int $score): array {
        if ($score >= 70) {
            return [
                'emoji' => '🔥',
                'label' => 'Actif',
     ...
     */
    public static function mettreAJourDerniereConnexion(int $id_user): bool {
        $pdo = config::getConnexion();
