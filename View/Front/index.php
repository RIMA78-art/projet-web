    ...
    <nav class="flex justify-between items-center px-8 h-16 w-full max-w-screen-2xl mx-auto">
        <div class="flex items-center gap-8">
            ...
            <div class="hidden md:flex gap-6 items-center">
                ...
                <a class="text-zinc-500 hover:text-green-600 font-['Manrope'] font-bold tracking-tight transition-colors duration-200 ease-in-out" href="index.php?action=nutrition-plan">Plan Nutritionnel</a>
                <a class="text-zinc-500 hover:text-green-600 font-['Manrope'] font-bold tracking-tight transition-colors duration-200 ease-in-out" href="index.php?action=top-meals">🏆 Top Repas</a>
            </div>
        </div>
...
<main class="pt-24 pb-20 px-8 max-w-screen-2xl mx-auto">
        ...
        <div class="flex flex-col md:flex-row gap-12 items-end">
            ...
            <div class="flex gap-4 pb-2">
                <div class="flex flex-col items-center p-4 bg-primary-container/20 rounded-xl min-w-[120px]">
                    <span class="text-2xl font-bold text-primary" id="meal-count"><?php echo $mealCount; ?></span>
    ...
    <!-- Lecteur Vidéo -->
    <section class="mb-16">
        <div class="bg-gradient-to-b from-primary/5 to-secondary/5 rounded-2xl overflow-hidden shadow-lg border border-primary/10">
            <div class="p-6 md:p-8">
                <div class="flex items-center gap-3 mb-6">
                    <span class="material-symbols-outlined text-primary text-3xl">play_circle</span>
                    <h2 class="text-3xl font-bold text-on-surface">Découvrez nos recettes en vidéo</h2>
                </div>
                <div class="bg-black rounded-xl overflow-hidden shadow-2xl">
                    <video 
                        ...
                        <source src="videos/What To Eat Before, During &amp; After Training For Max Muscle Growth.mp4" type="video/mp4">
                        Votre navigateur ne supporte pas le lecteur vidéo HTML5.
                    </video>
                </div>
                <p class="text-sm text-on-surface-variant mt-4 text-center">
                    💡 Placez vos fichiers vidéo (MP4, WebM) dans le dossier <code class="bg-zinc-100 px-2 py-1 rounded">/videos/</code>
                </p>
            </div>
        </div>
    </section>
    ...
    <section class="mb-12 sticky top-20 z-40 bg-surface/80 backdrop-blur-md py-4">
        <div class="flex flex-col gap-4">
            <div class="flex flex-col md:flex-row gap-4">
                <div class="flex-1">
                    <input id="meal-search" type="search" placeholder="Rechercher un repas par nom ou type" class="w-full rounded-md border border-outline-variant bg-white px-4 py-3 text-sm text-on-surface placeholder:text-on-surface-variant focus:border-primary focus:ring-primary">
                </div>
                <div class="flex items-center gap-2 text-sm text-on-surface-variant">
                    <span class="font-semibold">Résultats :</span>
                    <span id="visible-count" class="font-bold text-primary"><?php echo $mealCount; ?></span>
                </div>
            </div>
            <div class="flex flex-wrap gap-4" id="meal-filters">
                <button type="button" data-filter="all" class="meal-filter px-6 py-2 bg-primary text-on-primary rounded-md font-medium text-sm transition-all hover:translate-y-[-1px]">Tous les repas</button>
                <button type="button" data-filter="petit dejeuner" class="meal-filter px-6 py-2 bg-secondary-fixed text-on-secondary-fixed rounded-md font-medium text-sm hover:bg-secondary-fixed-dim transition-all">Petit déjeuner</button>
                <button type="button" data-filter="dejeuner" class="meal-filter px-6 py-2 bg-secondary-fixed text-on-secondary-fixed rounded-md font-medium text-sm hover:bg-secondary-fixed-dim transition-all">Déjeuner</button>
                <button type="button" data-filter="diner" class="meal-filter px-6 py-2 bg-secondary-fixed text-on-secondary-fixed rounded-md font-medium text-sm hover:bg-secondary-fixed-dim transition-all">Dîner</button>
            </div>
        </div>
    </section>

    <!-- Recipe Grid -->
    <section id="meals-grid" class="grid grid-cols-1 md:grid-cols-3 gap-8">
        <?php if (empty($meals)): ?>
        ...
        <?php else: ?>
            <?php foreach ($meals as $index => $meal): ?>
                <article
                    class="meal-card <?php echo $index === 0 ? 'md:col-span-2 md:row-span-2' : ''; ?> group cursor-pointer"
                    data-name="<?php echo htmlspecialchars($meal['nom'], ENT_QUOTES, 'UTF-8'); ?>"
    ...
    <div id="front-empty-state" class="hidden col-span-3 text-center py-12">
        <p class="text-on-surface-variant text-lg">Aucun repas ne correspond à votre recherche.</p>
    </div>
</main>
...
<script>
    ...
    const filterButtons = Array.from(document.querySelectorAll('.meal-filter'));
    const mealCards = Array.from(document.querySelectorAll('.meal-card'));
    const visibleCount = document.getElementById('visible-count');
    const totalCount = document.getElementById('meal-count');
    ...
    const updateFilterButtons = () => {
        filterButtons.forEach((button) => {
            ...
            button.classList.toggle('bg-secondary-fixed', !isActive);
            button.classList.toggle('text-on-secondary-fixed', !isActive);
        });
    };
    ...
    const applyFilters = () => {
        ...
        mealCards.forEach((card) => {
            const mealName = normalizeText(card.dataset.name);
            const mealType = normalizeText(card.dataset.type);
            const matchesSearch = !searchValue || mealName.includes(searchValue) || mealType.includes(searchValue);
            const matchesFilter = activeFilter === 'all' || mealType === activeFilter;
        ...
        totalCount.textContent = mealCards.length;
        emptyState.classList.toggle('hidden', count !== 0 || mealCards.length === 0);
    };
